        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .center) {
                ...
                TextField("Silakan Isi Nama Produk", text: $viewModel.name)
                    .disabled(!isEditing) // ✅ nonaktif kalau bukan mode edit
                    .padding(.vertical, 6)
                    .padding(.horizontal, 8)
                    .background(Color(.white))
                    .overlay(                      // outline hitam
                            RoundedRectangle(cornerRadius: 8)
                                .stroke(Color.black, lineWidth: 1)
                                .allowsHitTesting(false) // supaya overlay tidak blok interaksi
                        )
                    .cornerRadius(8)
                    .frame(maxWidth: .infinity)
                    .opacity(isEditing ? 1 : 0.7) // opsional: kasih efek "read-only"
            }
            .frame(maxWidth: .infinity)
