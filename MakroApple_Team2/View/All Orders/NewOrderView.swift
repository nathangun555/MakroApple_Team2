...
struct NewOrderView: View {
    ...
    //                .disabled(viewModel.isLoading || formPesanan.isEmpty)
                ...
                .onAppear {
                    ...
                    // Load shared images into the photo picker safely
                    if !sharedImages.isEmpty {
                        let maxPhotos = 3
                        let photosToUse = Array(sharedImages.prefix(maxPhotos))

                        // Pastikan array punya kapasitas cukup
                        while selectedImages.count < photosToUse.count {
                            selectedImages.append(nil)
                        }
                        while selectedItems.count < photosToUse.count {
                            selectedItems.append(nil)
                        }

                        // Isi gambar ke dalam slot
                        for (index, image) in photosToUse.enumerated() {
                            selectedImages[index] = image
                        }

                        print("📸 Auto-filled shared images into photo slots: \(photosToUse.count) images")
                    }
                }
                ...
                .onChange(of: sharedImages) { newImages in
                    print("📸 Detected sharedImages change: \(newImages.count) images")
                    ...
                    // Maksimal 3 foto
                    let photosToUse = Array(newImages.prefix(3))

                    // Isi images
                    selectedImages = photosToUse + Array(repeating: nil, count: max(0, 3 - photosToUse.count))

                    // Reset items (biar sinkron panjangnya meski belum ada PhotosPickerItem)
                    selectedItems = Array(repeating: nil, count: selectedImages.count)

                    print("🧩 Updated selectedImages: \(selectedImages.count) slots")
                }
