...
//struct MediumOrderWidgetView: View {
//    let orders: [OrderRecord]
//    let orderItems: [OrderItemRecord]
//
//    var body: some View {
...
//                    }
//
//                    // Ambil orderItems untuk order ini
//                    let itemsForOrder = Array(orderItems.filter { $0.orderId == order.id }.prefix(3))
//                    if !itemsForOrder.isEmpty {
//                        HStack(spacing: 4) {
//                            ForEach(itemsForOrder) { item in
...
//                                    .cornerRadius(3)
//                            }
//                            if orderItems.filter({ $0.orderId == order.id }).count > 3 {
//                                Text("+\(orderItems.filter({ $0.orderId == order.id }).count - 3) more")
//                                    .font(.caption2)
...
//        case "belum terbayar": return .red
//        case "diproses": return .orange
//        case "terkirim": return .blue
...
//
//                    // Produk yang dibeli
//                    let items = WidgetDataManager.shared.loadOrderItems()
...
//struct MediumWidgetView: View {
//    let orders: [OrderRecord]
//    let orderItems: [UUID: [OrderItemRecord]] // Mapping orderId -> item list
//    
...
//                   let items = orderItems[firstOrder.id], !items.isEmpty {
//                    
//                    let firstItem = items.first!
//                    let moreCount = items.count - 1
//                    
//                    dummyCard(
//                        time: "10.00",
//                        title: firstItem.productName + (moreCount > 0 ? " + \(moreCount) more" : ""),
...
//
//            // RIGHT SIDE: bisa isi dengan 2 item berikutnya misal
//            VStack(spacing: 8) {
//                if orders.count > 1 {
//                    ForEach(orders.dropFirst().prefix(2)) { order in
//                        if let items = orderItems[order.id], !items.isEmpty {
//                            let firstItem = items.first!
//                            let moreCount = items.count - 1
//                            dummyCard(
//                                time: "12.00",
...
//} timeline: {
//    SimpleEntry(date: .now, configuration: .preview(), orders: simpe.orders, orderItems: self.orderItems)
//}
